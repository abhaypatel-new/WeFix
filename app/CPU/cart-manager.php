...
class CartManager
{
    ...
    public static function add_to_cartcustom($request, $from_api = false)
    {
        $str = '';
        $variations = [];
        $price = 0;

        $user = Helpers::get_customproduct($request);
      ...
      //  print_r($product);die();
        ...
        if ($user == 'offline') {
            ...
            $cart = Cart::where(['custom_product_id' => $request->id, 'customer_id' => $user->id, 'variant' => $str])->first();
            if (isset($cart) == false) {
                $cart = new Cart();
            } else {
                return [
        ...
        $cart['color'] = $request->has('color') ? $request['color'] : null;
        $cart['custom_product_id'] = $product->id;
        $cart['choices'] = json_encode($choices);
        $imageses = json_decode($product->images);
        $cart['images'] =$imageses[0];
        //chek if out of stock
        // if ($product['current_stock'] < $request['quantity']) {
        //     return [
        //         'status' => 0,
...
//$tax = Helpers::tax_calculation($price, $product['tax'], 'percent');
       ...
       //   $cart['discount'] = Helpers::get_product_discount($product, $price);
        ...
        // $cart['shipping_cost'] = CartManager::get_shipping_cost_for_product_category_wise($product,$request['quantity']);
        if ($product->added_by == 'seller') {
            $cart['shop_info'] = Shop::where(['seller_id' => $product->seller_id])->first()->name;
        } else {
            $cart['shop_info'] = Helpers::get_business_settings('company_name');
        }
